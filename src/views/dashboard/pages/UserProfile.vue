...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      axios.get(process.env.VUE_APP_HOST_API + '/plans/id?id=' + idPlan).then(response => {
        this.name = response.data[0].name
        this.price = response.data[0].price
        this.limit = response.data[0].limit
      })
      axios.get(process.env.VUE_APP_HOST_API + '/users/userById?token=' + token).then(response => {
    ...
    methods: {
      updateUser: function () {
        ...
        if (this.$refs.form.validate()) {
          axios.post(process.env.VUE_APP_HOST_API + '/users/updateUser', {
            token: token,
            name: this.username,
            email: this.email,
            phone: this.phone,
